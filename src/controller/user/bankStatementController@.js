...
const { insertBankStatements, getBankStatements, insertBankTransaction, getFirmName, getLastBankStatement } = require("../../model/user/bankStatementsModel");
const { getJournalBalance, addJurnalEntry, checkJournalExist, getBankChargeLedgerBalance, removeFromOutstanding, isLedgerClientExists, updateJournalReconciliatioBankStatementDB } = require("../../model/admin/journalEntryModel");
...
const fetchBankStatements = async (req, res) => {
  try {
    ...
    bankStatements.forEach((statement) => {
      ...
      statement.daily_balance = Number(statement.daily_balance);
  ...
  try {
    if (transaction) {
      ...
      if (description.includes("Bank Charges") || description.includes("Interest")) {
        ...
        const finalDate = formattedDate.toLocaleDateString('en-CA');
      ...
      // const journalTransaction = await checkJournalExist({ date, payee_name, transaction_method, cheque_number, purpose, deposit_amount, disbursement_amount, ledger_client_id }, client_id, adminId, userId, role);
